...
class Bot:
	...
	def sendMessage(self, chat_id, text, metadata=[], parse_mode=None, message_id=None):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Bot._getURL(), data=dumps({"api_version":"5","auth": self.auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
				break
			except: continue
	...
	def getMessages(self, chat_id,min_id):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Bot._getURL(), data=dumps({"api_version":"5","auth": self.auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("messages")
				break
			except: continue
	...
	def sendPhoto(self, chat_id, file, size=[], thumbnail=None, caption=None, message_id=None):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Bot._SendFile(), data=dumps({"api_version":"5","auth": self.auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
				break
			except: continue
	...
	def getChatsUpdate(self):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Bot._getURL(), data=dumps({"api_version":"5","auth": self.auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("chats")
				break
			except: continue
	...
	def Send_Code(phone_number):
		...
		while 1:
			try:
				return loads(enc.decrypt(loads(request.urlopen(request.Request(Bot._getURL(), data=dumps({"api_version":"5","tmp_session":tmp,"data_enc":enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
				break
			except: continue

	def Send_Code_pass(phone_number, password):
		tmp = Bot._tmpGeneration()
		enc = encryption(tmp)
		inData = {
		    "method":"sendCode",
	...
	def registerDevice(Auth):
		enc = encryption(Auth)
		inData = {
		    "method":"registerDevice",
		    "input":{
		        "app_version":"WB_4.0.8",
		        "device_hash":"050712977485373640920451513153736",
		        "device_model":"ArianBOT",
		        "lang_code":"fa",
		...
		while 1:
			try:
				return loads(enc.decrypt(loads(request.urlopen(request.Request(Bot._getURL(), data=dumps({"api_version":"5","auth":Auth,"data_enc":enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
				break
			except: continue
